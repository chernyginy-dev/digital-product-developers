...
(function initGallery() {
  ...
  const scatter = [
    ...
    { x: -90, y:  50, r: -4, s: 0.90 },
    { x:  35, y:  70, r:  6, s: 0.90 },
    { x:  90, y:  35, r: -5, s: 0.90 },
  ...
  let hdrVis = false, introVis = false, footerVis = false;
  ...
  function onScroll() {
    ...
    rafPending = true;
    requestAnimationFrame(() => { rafPending = false; checkVisibility(); });
  }
  ...
  function checkVisibility() {
    ...
    if (hdrEl) {
      ...
      if (inV && !hdrVis)  { hdrVis = true;  requestAnimationFrame(() => fadeIn(hdrEl, -20, 0)); }
      else if (!inV && hdrVis) { hdrVis = false; requestAnimationFrame(() => fadeOut(hdrEl, -20)); }
    }
    ...
    if (introEl) {
      ...
      const inV = r.bottom > 80 && r.top < vh * 0.92;
      if (inV && !introVis)  { introVis = true;  requestAnimationFrame(() => fadeIn(introEl, 16, 80)); }
      else if (!inV && introVis) { introVis = false; requestAnimationFrame(() => fadeOut(introEl, 16)); }
    }
    ...
    if (footerEl) {
      ...
      if (inV && !footerVis)  { footerVis = true;  requestAnimationFrame(() => fadeIn(footerEl, 16, 200)); }
      else if (!inV && footerVis) { footerVis = false; requestAnimationFrame(() => fadeOut(footerEl, 16)); }
    }
  }
  ...
  cards.forEach((card) => {
    card.addEventListener("mousemove", (e) => {
      ...
      card.style.boxShadow  = "0 16px 48px rgba(0,0,0,0.14), 0 40px 80px rgba(0,0,0,0.10)";
    });
